...
{	
	...
	private static String makeAmpPage(String cateHtml, List<String> itmList, String cateUrl, String pageTitle) {
	...
	private static void writeToFile(String outDir, String cate, String curPage, String outStr) {
		//String path = ConfigParams.getPropertyValue("OUTPUT_HOME");
		// System.out.println("output=" + path);
		
		String fileName = "momo-amp." + cate + "." + curPage + ".html";
	    ...
	    System.out.println("output=" + outDir + fileName);
	...
	private static String getJsonPayload(String host, String cateCode, String curPage) {
    ...
    {	
		...
		logger.debug("Momo.Amp -- Process START --- ");
        
		// default value for params
		String postUrl = "http://www.momoshop.com.tw/api/moecapp/getCategoryGoodsV2";
        String host = "AMP";
        String cate = "4000700009";
        String curPage = "1";
        String outDir = "";
    	
        // 分類
    	if(args.length > 0) cate = args[0];
    	
    	//分頁
    	if(args.length > 1) curPage = args[1];
    	...
    	if(!ConfigParams.getPropertyValue("OUTPUT_HOME").equals("")) {
    		outDir = ConfigParams.getPropertyValue("OUTPUT_HOME");
    		logger.debug("Momo.Amp -- outDir={}", outDir);
    	}
        ...
        if(!jsonStr.equals("")) {
        	...
        	if(jsonResp.getBoolean("success")) { // success return
                ...
                String htmlStr = makeAmpPage(cateHtml, itmList, categoryUrl, pageTitle);
                writeToFile(outDir, cate, curPage, htmlStr);
        	}
        	else {
